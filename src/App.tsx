    ...
    const dispatcher = new StudioBridgeDispatcher({
      createShow: (identity: StudioSongIdentity) => {
        ...
        const snapshot: ShowProjectSnapshot = {
          ...
          savedAt: new Date().toISOString(),
          status: 'show',
          show: sanitizeShow({ ...EMPTY_SHOW, name: identity.songTitle, cues: [], groups: [], positionPalettes: [], recordings: [], externalTrack: { ...DEFAULT_EXTERNAL_TRACK_SYNC, songName: identity.songTitle, bpm: identity.bpm } }),
          patch: patch.map((fixture, index) => migratePatchedFixture(fixture, index, patch.length, stageSettings.dimensions)),
          stageElements: stageElements.map((element) => migrateStageElement(element, stageSettings.dimensions)),
          stageSettings: { ...stageSettings, dimensions: { ...stageSettings.dimensions } },
          looks: [...savedLooks]
      ...
      startEffect: (effectId) => {
        if (!EFFECT_PRESETS.some((effect) => effect.id === effectId)) throw new Error('Unknown LumaRig effect.');
        startEffect(effectId as EffectId);
      },
      stopEffect: () => stopEffect(),
      startRecording: (_songId, songTitle, bpm) => {
        setShowTrackName(songTitle);
        setRecordingTakeName(`${songTitle} · Studio Take ${(showFile.recordings?.length ?? 0) + 1}`);
        setEffectBpm(bpm);
        effectBpmRef.current = bpm;
        startShowRecording();
      },
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        // Native bridge is optional in browser/Vite development.
      }
    };
    const timer = window.setInterval(() => void poll(), 25);
    void poll();
    return () => { cancelled = true; window.clearInterval(timer); };
  }, [showLibrary, showFile, patch, stageElements, stageSettings, savedLooks, nextCue]);
