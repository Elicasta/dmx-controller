...
export default function App() {
  ...
  const [settings, setSettings] = useState<AppSettings>(() => ({ ...loadSettings(), visualizerArtNetEnabled: true, visualizerArtNetTarget: '127.0.0.1' }));
  const [artNetStatus, setArtNetStatus] = useState({ framesSent: 0, lastError: '' });
  const [lumaVizConnected, setLumaVizConnected] = useState(false);
  ...
  useEffect(() => {
    const probe = async () => {
      if (!settingsRef.current.visualizerArtNetEnabled) { setLumaVizConnected(false); return; }
      try { setLumaVizConnected(await invoke<boolean>('probe_lumaviz', { target: settingsRef.current.visualizerArtNetTarget.trim() || '127.0.0.1' })); }
      catch { setLumaVizConnected(false); }
    };
    void probe();
    const probeTimer = window.setInterval(probe, 1500);
    const timer = window.setInterval(() => {
      const status = outputRouterRef.current?.status().find((driver) => driver.id === 'artnet');
      if (status) setArtNetStatus({ framesSent: status.framesSent, lastError: status.lastError ?? '' });
    }, 250);
    return () => { window.clearInterval(timer); window.clearInterval(probeTimer); };
  }, []);
      ...
      {workspace === 'setup' && <section className="console-workspace setup-console">
        ...
        <div className="setup-center console-center">
          ...
          {setupView === 'settings' && <div className="setup-scroll-area settings-console">
            ...
            <section className="console-panel connection-console visualizer-output-console"><header><div><span>NETWORK DMX · ART-NET</span><h2>LumaViz Link</h2></div><b className={settings.visualizerArtNetEnabled && !artNetStatus.lastError ? 'healthy' : ''}>{artNetStatus.lastError ? 'ERROR' : lumaVizConnected ? 'CONNECTED' : settings.visualizerArtNetEnabled ? 'SENDING' : 'Off'}</b></header><p>LumaRig automatically sends the final resolved DMX frame to LumaViz on this Mac at 127.0.0.1:6454.</p><div className="connection-diagnostics"><strong>{lumaVizConnected ? 'LUMAVIZ ACKNOWLEDGED' : 'WAITING FOR LUMAVIZ ACK'} · FRAMES SENT {artNetStatus.framesSent}</strong>{artNetStatus.lastError && <small>{artNetStatus.lastError}</small>}</div><label className="inspector-toggle"><span>Enable Art-Net output</span><input type="checkbox" checked={settings.visualizerArtNetEnabled} onChange={(event) => setSettings((current) => ({ ...current, visualizerArtNetEnabled: event.target.checked }))} /></label><label><span>Target IPv4 address</span><input value={settings.visualizerArtNetTarget} placeholder="127.0.0.1" onChange={(event) => setSettings((current) => ({ ...current, visualizerArtNetTarget: event.target.value }))} /></label><small>Same computer / LumaViz: 127.0.0.1 · Another machine or Art-Net node: enter its LAN IPv4 address. Broadcast is also supported with 255.255.255.255.</small><div className="settings-actions"><button onClick={() => setSettings((current) => ({ ...current, visualizerArtNetTarget: '127.0.0.1', visualizerArtNetEnabled: true }))}>LumaViz · This Computer</button><button onClick={() => setSettings((current) => ({ ...current, visualizerArtNetTarget: '255.255.255.255', visualizerArtNetEnabled: true }))}>Broadcast LAN</button><button onClick={() => setSettings((current) => ({ ...current, visualizerArtNetEnabled: false }))}>Stop Art-Net</button></div></section>
