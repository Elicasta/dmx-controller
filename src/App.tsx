...
import { OperatorMenuBar } from './components/OperatorMenuBar';
...
type Workspace = 'build' | 'create' | 'show' | 'live' | 'settings';
type SettingsView = 'connections' | 'appearance' | 'safety';
type SetupView = 'fixtures' | 'groups' | 'stage';
...
function showSnapshotFileName(snapshot: ShowProjectSnapshot) {
  return `${snapshot.status}-${snapshot.id.replace(/[^a-z0-9_-]/gi, '-')}.lumarig.json`;
}
...
function loadShowLibrary(): ShowProjectSnapshot[] {
  ...
  try {
    ...
    return parsed.filter(isShowProjectSnapshot).slice(0, 100);
  } catch {
    return [];
  }
}
...
export default function App() {
  const [workspace, setWorkspace] = useState<Workspace>(() => initialConsoleValue('workspace', ['build', 'create', 'show', 'live', 'settings'], 'create'));
  const [settingsView, setSettingsView] = useState<SettingsView>('connections');
  const [theme, setTheme] = useState<'studio' | 'slate' | 'contrast'>(() => {
    const stored = localStorage.getItem('lumarig.appearance.theme');
    return stored === 'slate' || stored === 'contrast' ? stored : 'studio';
  });
  const [railWidths, setRailWidths] = useState<Record<Workspace, number>>(() => {
    try { return { build: 260, create: 260, show: 210, live: 190, settings: 210, ...JSON.parse(localStorage.getItem('lumarig.layout.rails') || '{}') }; }
    catch { return { build: 260, create: 260, show: 210, live: 190, settings: 210 }; }
  });
  useEffect(() => { localStorage.setItem('lumarig.appearance.theme', theme); }, [theme]);
  useEffect(() => { localStorage.setItem('lumarig.layout.rails', JSON.stringify(railWidths)); }, [railWidths]);
  const resizeRail = (event: ReactPointerEvent<HTMLDivElement>) => {
    event.preventDefault();
    const page = workspace;
    const startX = event.clientX;
    const startWidth = railWidths[page];
    const move = (next: PointerEvent) => setRailWidths((current) => ({ ...current, [page]: Math.max(156, Math.min(350, startWidth + next.clientX - startX)) }));
    const stop = () => { window.removeEventListener('pointermove', move); window.removeEventListener('pointerup', stop); };
    window.addEventListener('pointermove', move);
    window.addEventListener('pointerup', stop, { once: true });
  };
  const railHandle = <div className="workspace-rail-resize" role="separator" aria-label="Resize navigation" aria-orientation="vertical" aria-valuemin={156} aria-valuemax={350} aria-valuenow={railWidths[workspace]} tabIndex={0} onPointerDown={resizeRail} onKeyDown={(event) => { if (event.key === 'ArrowLeft' || event.key === 'ArrowRight') { event.preventDefault(); setRailWidths((current) => ({ ...current, [workspace]: Math.max(156, Math.min(350, current[workspace] + (event.key === 'ArrowRight' ? 16 : -16))) })); } }} />;
  const [setupView, setSetupView] = useState<SetupView>(() => initialConsoleValue('setup', ['fixtures', 'groups', 'stage'], 'stage'));
  ...
  const [showLibraryFilter, setShowLibraryFilter] = useState<'all' | ShowProjectSnapshot['status']>('all');
  const [settings, setSettings] = useState<AppSettings>(loadSettings);
  const [showLibraryDirectoryDraft, setShowLibraryDirectoryDraft] = useState(() => loadSettings().showLibraryDirectory);
  ...
  const [createColorFolder, setCreateColorFolder] = useState('All folders');
  const [positionSection, setPositionSection] = useState<'all' | 'spatial' | 'absolute' | 'looks'>('all');
  ...
  useEffect(() => setShowLibraryDirectoryDraft(settings.showLibraryDirectory), [settings.showLibraryDirectory]);
  useEffect(() => {
    if (settings.showLibraryDirectory.trim()) return;
    void invoke<string>('default_show_library_directory')
      .then((directory) => setSettings((current) => current.showLibraryDirectory ? current : { ...current, showLibraryDirectory: directory }))
      .catch(() => { /* browser preview */ });
  }, [settings.showLibraryDirectory]);
  useEffect(() => {
    const directory = settings.showLibraryDirectory.trim();
    if (!directory) return;
    let cancelled = false;
    void invoke<string[]>('read_show_library_snapshots', { directory }).then((documents) => {
      if (cancelled) return;
      const diskSnapshots = documents.flatMap((document) => {
        try { const parsed: unknown = JSON.parse(document); return isShowProjectSnapshot(parsed) ? [parsed] : []; }
        catch { return []; }
      });
      setShowLibrary((current) => {
        const merged = new Map(current.map((item) => [item.id, item]));
        diskSnapshots.forEach((item) => merged.set(item.id, item));
        return [...merged.values()].sort((a, b) => b.savedAt.localeCompare(a.savedAt)).slice(0, 100);
      });
    }).catch(() => { /* folder may not exist in a browser preview */ });
  ...
  async function resolveShowLibraryDirectory() {
    const configured = settings.showLibraryDirectory.trim();
    if (configured) return configured;
    const directory = await invoke<string>('default_show_library_directory');
    setSettings((current) => ({ ...current, showLibraryDirectory: directory }));
    return directory;
  }

  async function persistShowSnapshot(snapshot: ShowProjectSnapshot) {
    const directory = await resolveShowLibraryDirectory();
    await invoke<string>('write_show_library_snapshot', {
      directory,
      fileName: showSnapshotFileName(snapshot),
      contents: JSON.stringify(snapshot, null, 2)
    });
  }

  async function refreshShowLibraryFolder() {
    try {
      const directory = await resolveShowLibraryDirectory();
      const documents = await invoke<string[]>('read_show_library_snapshots', { directory });
      const snapshots = documents.flatMap((document) => {
        try { const parsed: unknown = JSON.parse(document); return isShowProjectSnapshot(parsed) ? [parsed] : []; }
        catch { return []; }
      });
      setShowLibrary(snapshots.sort((a, b) => b.savedAt.localeCompare(a.savedAt)).slice(0, 100));
      setMessage(`Show library refreshed from ${directory}.`);
    } catch (error) {
      setMessage(`Could not refresh the show library folder: ${String(error)}`);
    }
  }

  async function revealShowLibraryFolder() {
    try {
      const directory = await resolveShowLibraryDirectory();
      await invoke('reveal_show_library_directory', { directory });
    } catch (error) {
      setMessage(`Could not open the show library folder: ${String(error)}`);
    }
  }

  async function restoreDefaultShowLibraryFolder() {
    try {
      const directory = await invoke<string>('default_show_library_directory');
      setSettings((current) => ({ ...current, showLibraryDirectory: directory }));
      setMessage(`Show library folder set to ${directory}.`);
    } catch (error) {
      setMessage(`Could not set the default show library folder: ${String(error)}`);
    }
  }
  ...
  function saveShowProject(status: 'template' | 'draft' | 'show' = 'show') {
    ...
    setShowLibrary((current) => [snapshot, ...current.filter((item) => item.id !== snapshot.id)].slice(0, 100));
    setMessage(`Saving ${cleanName} to the show library folder…`);
    void persistShowSnapshot(snapshot)
      .then(() => setMessage(`${cleanName} saved as ${status === 'template' ? 'a template' : status === 'draft' ? 'a draft' : 'a service show'} in the show library folder.`))
      .catch((error) => setMessage(`${cleanName} is autosaved locally, but the show-library file could not be written: ${String(error)}`));
  }
  ...
  function deleteShowProject(id: string) {
    ...
    if (settings.showLibraryDirectory.trim()) {
      void invoke('remove_show_library_snapshot', { directory: settings.showLibraryDirectory.trim(), fileName: showSnapshotFileName(item) })
        .catch((error) => setMessage(`${item.name} was removed locally, but its file could not be deleted: ${String(error)}`));
    }
  ...
  function recapturePositionPalette(palette: PositionPalette) {
    updatePositionPalette(palette);
  }
  ...
  function changeMidiTriggerValue(id: string, triggerValue: number) {
    const nextValue = Math.max(1, Math.min(127, Math.round(triggerValue || 1)));
    setMidiMappings((current) => current.map((mapping) => mapping.id === id ? { ...mapping, triggerValue: nextValue } : mapping));
  }

  function restoreCommonMidiDefaults() {
    setMidiMappings(DEFAULT_MIDI_MAPPINGS.map((mapping) => ({ ...mapping })));
    setMidiLearnMappingId(null);
    setMessage('Common MIDI defaults restored: CC1 GO, CC2 BACK, CC3 BLACKOUT and CC7 MASTER on channel 1.');
  }
  ...
  midiActionRef.current = (event) => {
    ...
    matches.forEach((mapping) => {
      ...
      const pressed = value >= mapping.triggerValue;
  ...
  function applyGroupMaster(group: FixtureGroup, percent: number) {
    ...
    const previous = groupLevelsRef.current[group.id] ?? groupMasters[group.id] ?? group.masterDefault;
    groupLevelsRef.current[group.id] = value;
    setGroupMasters((current) => ({ ...current, [group.id]: value }));
    void dispatchControl({ type: 'group.level.adjust', groupName: group.name, previous, value });
    setMessage(`${group.name} at ${Math.round(value)}%. Fixture faders updated together.`);
  }
  ...
  return (
    <main className={`console-app workspace-${workspace} theme-${theme} ${dmxStatus.blackout ? 'blackout-is-active' : ''}`} style={{ '--workspace-rail-width': `${railWidths[workspace]}px` } as import('react').CSSProperties}>
      <header className="console-header">
        ...
        <div className="console-primary-navigation"><OperatorMenuBar menus={[
          { label: 'File', items: [{ label: 'New Show', action: newShowProject }, { label: 'Save Service Show', action: () => saveShowProject('show'), separator: true }, { label: 'Save Draft', action: () => saveShowProject('draft') }, { label: 'Save Template', action: () => saveShowProject('template') }, { label: 'Show Library', action: () => { setWorkspace('show'); setShowMode('library'); } }, { label: 'Open Save Folder', action: () => void revealShowLibraryFolder() }] },
          { label: 'Show', items: [{ label: 'Cues', action: () => { setWorkspace('show'); setShowMode('cues'); } }, { label: 'Timeline', action: () => { setWorkspace('show'); setShowMode('timeline'); } }, { label: 'Tracks', action: () => { setWorkspace('show'); setShowMode('tracks'); } }, { label: 'Capture Cue', action: captureCue, separator: true }, { label: 'Go / Next Cue', action: goNextCue, disabled: !nextCue }, { label: 'Back', action: goPreviousCue }] },
          { label: 'Fixtures', items: [{ label: 'Patch Fixtures', action: () => { setWorkspace('build'); setSetupView('fixtures'); } }, { label: 'Groups', action: () => { setWorkspace('build'); setSetupView('groups'); } }, { label: 'Stage', action: () => { setWorkspace('build'); setSetupView('stage'); } }, { label: 'Select All', action: selectAllFixtures, separator: true }, { label: 'Clear Selection', action: clearFixtureSelection }] },
          { label: 'View', items: (['build', 'create', 'show', 'live', 'settings'] as Workspace[]).map((page) => ({ label: page[0].toUpperCase() + page.slice(1), action: () => setWorkspace(page) })) },
          { label: 'Settings', items: [{ label: 'Connections', action: () => { setWorkspace('settings'); setSettingsView('connections'); } }, { label: 'Appearance', action: () => { setWorkspace('settings'); setSettingsView('appearance'); } }, { label: 'Safety', action: () => { setWorkspace('settings'); setSettingsView('safety'); } }, { label: 'MIDI Mapper', action: () => setMidiManagerOpen(true), separator: true }] }
        ]} /><nav className="console-workspace-tabs" aria-label="Workspace">{(['build', 'create', 'show', 'live', 'settings'] as Workspace[]).map((item) => <button key={item} className={workspace === item ? 'active' : ''} onClick={() => setWorkspace(item)}>{item.toUpperCase()}</button>)}</nav></div>
        <div className="console-header-status">
          ...
          <button className={`connection-pill ${dmxStatus.connected ? 'online' : ''}`} onClick={() => { setWorkspace('settings'); setSettingsView('connections'); }}><i /><span><strong>DMX</strong><small>{dmxStatus.connected ? 'CONNECTED' : 'VIRTUAL'}</small></span></button>
      ...
      {(workspace === 'build' || workspace === 'settings') && <section className={`console-workspace setup-console ${workspace === 'settings' ? 'settings-workspace' : ''}`}>
        <aside className="workspace-left-rail"><nav className="workspace-rail-nav" aria-label={workspace === 'settings' ? 'Settings sections' : 'Build sections'}><header>{workspace === 'settings' ? 'SETTINGS' : 'BUILD'}</header>{workspace === 'settings' ? (['connections', 'appearance', 'safety'] as SettingsView[]).map((id) => <button key={id} className={settingsView === id ? 'active' : ''} onClick={() => setSettingsView(id)}>{id[0].toUpperCase() + id.slice(1)}</button>) : (['fixtures', 'groups', 'stage'] as SetupView[]).map((id) => <button key={id} className={setupView === id ? 'active' : ''} onClick={() => setSetupView(id)}>{id[0].toUpperCase() + id.slice(1)}</button>)}</nav>
        {workspace === 'build' && <FixtureBrowser
          patch={patch}
        ...
        <div className="setup-center console-center">
          {workspace === 'settings' && settingsView === 'appearance' && <div className="setup-scroll-area settings-console"><section className="console-panel appearance-panel"><header><div><span>APPEARANCE</span><h2>Operator theme</h2></div></header><p>Choose a comfortable contrast for your room. Your choice stays on this Mac.</p><div className="theme-choices">{([['studio','Studio','The familiar LumaRig dark console.'],['slate','Slate','Cooler surfaces and softer separation.'],['contrast','High Contrast','Sharper edges and brighter labels.']] as const).map(([id,label,description]) => <button key={id} className={theme === id ? 'active' : ''} onClick={() => setTheme(id)}><span className={`theme-preview theme-preview-${id}`} /><strong>{label}</strong><small>{description}</small></button>)}</div></section><section className="console-panel appearance-panel"><header><div><span>LAYOUT</span><h2>Workspace size</h2></div></header><p>Drag the divider beside any left rail to resize it. Use the arrow keys when the divider has focus.</p><button onClick={() => setRailWidths({ build: 260, create: 260, show: 210, live: 190, settings: 210 })}>Reset panel widths</button></section></div>}
          {workspace === 'settings' && settingsView === 'safety' && <div className="setup-scroll-area settings-console"><section className="console-panel safety-settings"><header><div><span>PERFORMANCE SAFETY</span><h2>Guardrails</h2></div></header><label><span>Grand master limit</span><input type="range" min="10" max="100" value={settings.masterLimit} onChange={(event) => setSettings((current) => ({ ...current, masterLimit: Number(event.target.value) }))} /><b>{settings.masterLimit}%</b></label><label><span>Confirm blackout release</span><input type="checkbox" checked={settings.confirmBlackoutRelease} onChange={(event) => setSettings((current) => ({ ...current, confirmBlackoutRelease: event.target.checked }))} /></label></section></div>}
          {workspace === 'build' && <>{setupView === 'stage' && <>
          ...
          {workspace === 'build' && setupView === 'fixtures' && <div className="setup-scroll-area">
          ...
          {workspace === 'settings' && settingsView === 'connections' && <div className="setup-scroll-area settings-console">
            ...
            <section className="console-panel midi-mapping-console"><header><div><span>MIDI ASSIGNER</span><h2>Map controls</h2></div><b>{midiMappings.length} mappings</b></header><MidiMappingPanel compact mappings={midiMappings} controls={midiControls} groups={midiControlGroups} newTarget={newMidiTarget} learningId={midiLearnMappingId} onNewTarget={setNewMidiTarget} onAdd={()=>beginMidiAssignment()} onLearn={setMidiLearnMappingId} onRemove={removeMidiAssignment} onTarget={changeMidiAssignmentTarget} onTrigger={changeMidiTriggerValue} onRestoreDefaults={restoreCommonMidiDefaults} onOpenManager={()=>setMidiManagerOpen(true)}/></section>
        ...
        {workspace === 'build' && <aside className="console-inspector setup-inspector">
      ...
      {workspace === 'create' && <section className={`console-workspace program-console create-console-v3 ${programMode === 'stage' ? '' : 'create-focus-mode'}`}>
        <aside className="create-left-rail">
          <nav className="workspace-rail-nav" aria-label="Create sections"><header>CREATE</header>{([['stage','Programmer'],['looks','Looks'],['fx','FX'],['colors','Color Palettes'],['media','Media'],['presets','Positions & Looks']] as Array<[ProgramMode,string]>).map(([id,label]) => <button key={id} className={programMode === id ? 'active' : ''} onClick={() => setProgramMode(id)}>{label}</button>)}</nav>
          {(programMode === 'colors' || programMode === 'presets') && <nav className="create-folder-nav"><header>{programMode === 'colors' ? 'COLOR FOLDERS' : 'POSITIONS & LOOKS'}</header>{programMode === 'colors' ? <><button className={createColorFolder === 'All folders' ? 'active' : ''} onClick={() => setCreateColorFolder('All folders')}>All colors <b>{showFile.colorPalettes?.length ?? 0}</b></button>{[...new Set((showFile.colorPalettes ?? []).map((item) => item.folder.trim()).filter(Boolean))].sort().map((folder) => <button key={folder} className={createColorFolder === folder ? 'active' : ''} onClick={() => setCreateColorFolder(folder)}>▸ {folder} <b>{showFile.colorPalettes?.filter((item) => item.folder.trim() === folder).length}</b></button>)}{(showFile.colorPalettes ?? []).some((item) => !item.folder.trim()) && <button className={createColorFolder === 'Unfiled' ? 'active' : ''} onClick={() => setCreateColorFolder('Unfiled')}>Unfiled</button>}</> : (['all','spatial','absolute','looks'] as const).map((item) => <button key={item} className={positionSection === item ? 'active' : ''} onClick={() => setPositionSection(item)}>{item === 'all' ? 'All presets' : item === 'spatial' ? 'Target positions' : item === 'absolute' ? 'Pan / tilt positions' : 'Looks'} <b>{item === 'looks' ? allLooks.length : item === 'all' ? (showFile.positionPalettes?.length ?? 0) + allLooks.length : showFile.positionPalettes?.filter((palette) => palette.kind === item).length}</b></button>)}</nav>}
          <FixtureBrowser patch={patch} groups={fixtureGroups} search={fixtureSearch} onSearchChange={setFixtureSearch} onSelectAll={selectAllFixtures} onClearSelection={clearFixtureSelection} onSelectFixture={selectFixtureFromConsole} onSelectGroup={selectFixtureGroup} selectedGroupId={selectedGroupId} />{railHandle}
        </aside>
        <div className="program-center console-center">
          <nav className="workspace-subtabs program-subtabs">{([
            ['stage', 'Programmer'], ['looks', 'Looks'], ['fx', 'FX'], ['colors', 'Color Palettes'], ['media', 'Media'], ['presets', 'Positions & Looks']
          ...
          {programMode === 'stage' && <div className="programmer-v3">
            ...
            <div className="programmer-attribute-deck programmer-attribute-deck-v4">
              ...
              <div className={`mover-attribute-group ${moverControlsOpen ? 'open' : ''}`}><button className="mover-attribute-toggle" onClick={() => setMoverControlsOpen((current) => !current)} aria-expanded={moverControlsOpen}>{moverControlsOpen ? '▾' : '▸'} MOVING LIGHT CONTROLS <small>{selectedMovingFixtures.length} MOVERS · {moverControlsOpen ? 'COLLAPSE' : 'EXPAND'}</small></button><div className="mover-attribute-content">
          ...
          {programMode === 'presets' && <div className="create-focus-view position-library-view"><header><div><span>POSITIONS & LOOKS</span><h2>Reusable stage states</h2></div></header><section className="position-save-bar"><label><span>POSITION NAME</span><input value={positionPaletteName} placeholder={selectedTarget?.name || `Position ${(showFile.positionPalettes?.length ?? 0) + 1}`} onChange={(event)=>setPositionPaletteName(event.target.value)}/></label><label><span>SAVE MODE</span><select value={positionPaletteKind} onChange={(event)=>setPositionPaletteKind(event.target.value as 'spatial'|'absolute')}><option value="spatial">Target-based · follows stage geometry</option><option value="absolute">Pan / tilt values · exact fixture positions</option></select></label><button className="console-primary" disabled={!selectedMovingFixtures.length} onClick={savePositionPalette}>＋ Save Position</button></section><p className="position-library-help">Target-based positions aim selected moving fixtures at a named point such as Center Stage. Pan / tilt positions store the exact mover values you have now.</p><section className="preset-bank-v3">{positionSection !== 'looks' && <div><header><h3>POSITION PALETTES</h3><small>{showFile.positionPalettes?.filter((palette) => positionSection === 'all' || palette.kind === positionSection).length ?? 0} SAVED</small></header>{showFile.positionPalettes?.filter((palette) => positionSection === 'all' || palette.kind === positionSection).map((palette) => <article className="position-preset-card" key={palette.id}><button onClick={() => void runPositionPalette(palette)}><span>{palette.kind === 'spatial' ? 'TARGET' : 'PAN / TILT'}</span><strong>{palette.name}</strong><small>{palette.kind === 'spatial' ? `${palette.targetName} · ${palette.arrangement.replaceAll('-', ' ')}` : `${palette.positions.length} fixture${palette.positions.length === 1 ? '' : 's'} · exact values`}</small></button><div><button onClick={() => void recapturePositionPalette(palette)}>Recapture</button><button className="danger-button" onClick={() => deletePositionPalette(palette.id)}>Delete</button></div></article>)}{!showFile.positionPalettes?.some((palette) => positionSection === 'all' || palette.kind === positionSection) && <p>No positions in this section yet. Select a moving fixture and save one above.</p>}</div>}{(positionSection === 'all' || positionSection === 'looks') && <div><header><h3>LOOK PRESETS</h3><small>{allLooks.length} AVAILABLE</small></header><div className="look-preset-grid">{allLooks.map((look) => <button key={look.id} onClick={() => runLook(look)}><i style={{background:lookSwatch(look.values)}}/><strong>{look.name}</strong></button>)}</div></div>}</section></div>}
        </div>

        {programMode === 'stage' && <EffectsPanel title="FX / SELECTED TARGET" targetName={programEffectName} fixtures={programEffectFixtures} activeEffect={activeEffect} bpm={effectBpm} depth={effectDepth} disabled={false} onBpmChange={(value) => { setEffectBpm(value); effectBpmRef.current = value; setTempoSource('manual'); }} onDepthChange={(value) => { setEffectDepth(value); effectDepthRef.current = value; }} onStart={(effect) => toggleEffect(effect, programEffectFixtures.map((fixture) => fixture.id))} onPress={(effect) => startMomentaryEffect(effect, programEffectFixtures.map((fixture) => fixture.id))} onRelease={releaseMomentaryEffect} onStop={() => stopEffect()} />}
      </section>}

      {workspace === 'show' && <section className="show-console console-workspace-wide show-console-v3">
        <aside className="workspace-left-rail"><nav className="workspace-rail-nav" aria-label="Show sections"><header>SHOW</header>{([
          ['cues','Cues'],['timeline','Timeline'],['tracks','Tracks'],['library','Show Library'],['sync','MIDI & Sync'],['recordings','Recordings']
        ] as Array<[ShowMode,string]>).map(([id,label]) => <button key={id} className={showMode === id ? 'active' : ''} onClick={() => setShowMode(id)}>{label}</button>)}</nav>{showMode === 'library' && <nav className="workspace-rail-nav library-rail-nav" aria-label="Library folders"><header>FOLDERS</header>{(['all','template','draft','show'] as const).map((status) => <button key={status} className={showLibraryFilter === status ? 'active' : ''} onClick={() => setShowLibraryFilter(status)}>{status === 'all' ? 'All Shows' : status === 'show' ? 'Service Shows' : `${status[0].toUpperCase()}${status.slice(1)}s`} <b>{status === 'all' ? showLibrary.length : showLibrary.filter((item) => item.status === status).length}</b></button>)}<button onClick={() => void revealShowLibraryFolder()}>Open Save Folder ↗</button></nav>}{railHandle}</aside>

        {showMode === 'cues' && <div className="show-cue-layout">
          <aside className="cue-list-console">
            <header><span>CUE LIST</span><button onClick={captureCue}>＋ Capture</button></header>
            {showFile.cues.length ? showFile.cues.map((cue,index) => <article className={activeCueId === cue.id ? 'active' : ''} key={cue.id} style={{'--cue-color':cue.color ?? '#55e98d'} as import('react').CSSProperties}>
              <button className="cue-line" onClick={() => runCue(cue)}><b>{String(cue.number).padStart(2,'0')}</b><i style={{background:cue.color ?? '#55e98d'}}/><span><strong>{cue.name}</strong><small>{cue.fadeMs ? `${cue.fadeMs/1000}s fade` : 'Snap'}{cue.followMs ? ` · follow ${cue.followMs/1000}s` : ''}</small></span></button>
              <div><button disabled={index===0} onClick={() => setShowFile((current)=>({...current,cues:moveCue(current.cues,cue.id,-1)}))}>↑</button><button disabled={index===showFile.cues.length-1} onClick={() => setShowFile((current)=>({...current,cues:moveCue(current.cues,cue.id,1)}))}>↓</button><button onClick={() => deleteCue(cue.id)}>×</button></div>
            </article>) : <div className="empty-cues"><strong>No cues yet</strong><span>Build a look in CREATE, then capture it here.</span><button onClick={() => setWorkspace('create')}>Open CREATE</button></div>}
          </aside>

          <main className="cue-preview-console">
            <header><span>{directStatus.clients > 0 ? 'LUMAVIZ LIVE PREVIEW' : 'STAGE / CUE PREVIEW'}</span><b>{activeCue?.name ?? 'Live output'}</b></header>
            <div className={`show-viz-preview ${liveLumaVizPreview ? 'linked external-feed' : directStatus.clients > 0 ? 'linked' : ''}`}>{liveLumaVizPreview ? <img src={liveLumaVizPreview.dataUrl} alt={`LumaViz ${liveLumaVizPreview.view ?? 'live'} preview`} /> : renderStagePreview()}</div>
          </main>

          <aside className="cue-inspector-console">
            <header><span>CUE INSPECTOR</span><strong>{activeCue?.name ?? 'New cue'}</strong></header>
            {activeCue ? <>
              <label><span>Cue Name</span><input value={activeCue.name} onChange={(event)=>updateCueProperties(activeCue.id,{name:event.target.value})}/></label>
              <label><span>Cue Color</span><input type="color" value={activeCue.color ?? '#55e98d'} onChange={(event)=>updateCueProperties(activeCue.id,{color:event.target.value})}/></label>
            ...
            </> : <><label><span>New Cue Name</span><input value={cueName} placeholder={`Cue ${showFile.cues.length+1}`} onChange={(event)=>setCueName(event.target.value)}/></label><label><span>Fade In</span><select value={cueFadeMs} onChange={(event)=>setCueFadeMs(Number(event.target.value))}>{FADE_TIMES.map((time)=><option key={time} value={time}>{time===0?'Snap':`${time/1000}s`}</option>)}</select></label><button className="console-primary" onClick={captureCue}>Capture Current Look</button></>}
            <label><span>Show Notes</span><textarea value={showFile.notes ?? ''} placeholder="Set list, transitions, safety notes…" onChange={(event)=>setShowFile((current)=>({...current,notes:event.target.value}))}/></label>
          </aside>

          <div className="cue-transport-console"><button onClick={goPreviousCue} disabled={!showFile.cues.length}>BACK</button><span style={{'--cue-color':activeCue?.color ?? '#55e98d'} as import('react').CSSProperties}><small>CURRENT · {activeCue?.number ?? '—'}</small><strong>{activeCue?.name ?? 'Ready'}</strong></span><button className="giant-go" onClick={goNextCue} disabled={!nextCue}>GO<small>{nextCue?.name ?? 'End'}</small></button><span style={{'--cue-color':nextCue?.color ?? '#82aaff'} as import('react').CSSProperties}><small>NEXT · {nextCue?.number ?? '—'}</small><strong>{nextCue?.name ?? 'End of show'}</strong></span></div>
        </div>}
        ...
        {showMode === 'library' && <div className="show-library-layout"><aside className="show-library-rail"><header>SHOW LIBRARY <small>{showLibrary.length} SAVED</small></header><nav className="show-library-filters">{(['all','template','draft','show'] as const).map((status)=><button key={status} className={showLibraryFilter===status?'active':''} onClick={()=>setShowLibraryFilter(status)}>{status==='all'?'All shows':status==='show'?'Service Shows':`${status[0].toUpperCase()}${status.slice(1)}s`}<b>{status==='all'?showLibrary.length:showLibrary.filter((item)=>item.status===status).length}</b></button>)}</nav><div className="show-library-rail-folder"><strong>LOCAL SAVE FOLDER</strong><small>{showLibraryDirectoryDraft}</small><button onClick={()=>void revealShowLibraryFolder()}>Open in Finder ↗</button></div></aside><div className="show-library-console show-library-v3">
          ...
          <section className="show-library-storage"><div><span>SAVE FOLDER</span><input aria-label="Show library folder" value={showLibraryDirectoryDraft} placeholder="Documents/LumaRig Shows" onChange={(event)=>setShowLibraryDirectoryDraft(event.target.value)}/></div><button onClick={()=>setSettings((current)=>({...current,showLibraryDirectory:showLibraryDirectoryDraft.trim()}))}>Use Folder</button><button onClick={()=>void restoreDefaultShowLibraryFolder()}>Default</button><button onClick={()=>void revealShowLibraryFolder()}>Open Folder</button><button onClick={()=>void refreshShowLibraryFolder()}>Refresh</button></section>
          <div className="show-library-folders">{(['template','draft','show'] as const).filter((status)=>showLibraryFilter==='all'||showLibraryFilter===status).map((status)=>{const items=showLibrary.filter((item)=>item.status===status);return <section className={`show-library-folder ${status}`} key={status}><header><span>{status==='show'?'SERVICE SHOWS':`${status.toUpperCase()}S`}</span><b>{items.length}</b></header><div>{items.map((item)=><article key={item.id}><div><span className={item.status}>{item.status.toUpperCase()}</span><strong>{item.name}</strong><small>{new Date(item.savedAt).toLocaleString()}</small><small>R{item.revision ?? 1} · {item.show.cues.length} cues · {item.patch.length} fixtures</small></div><div><button className="console-primary" onClick={()=>loadShowProject(item)}>Load</button><button className="danger-button" onClick={()=>deleteShowProject(item.id)}>Delete</button></div></article>)}{!items.length&&<p>No {status==='show'?'service shows':`${status}s`} in this folder.</p>}</div></section>;})}</div>
        </div></div>}
      ...
      {workspace === 'live' && <section className={`live-console live-console-v3 ${liveView === 'performance' ? 'controller-live-view' : ''}`}>
        <aside className="workspace-left-rail"><nav className="workspace-rail-nav" aria-label="Live sections"><header>LIVE</header>{([['performance','Controller'],['overrides','Fixture Overrides'],['groups','Groups'],['masters','Master Controls'],['shortcuts','Shortcuts'],['settings','System Status']] as Array<[LiveView,string]>).map(([id,label]) => <button key={id} className={liveView === id ? 'active' : ''} onClick={() => setLiveView(id)}>{label}</button>)}</nav>{railHandle}</aside>
        ...
        {liveView === 'performance' && <DesktopLiveController
          ...
          dmxConnected={dmxStatus.connected}
          master={globalMaster}
          fxSpeed={Math.round(((tempoSource==='midi'&&midiBpm?midiBpm:effectBpm)-30)/2.1)}
          fxDepth={effectDepth}
          fixtures={patch.map((fixture)=>{const values=fixtureValues(outputUniverse,fixture);return {id:fixture.id,name:fixture.name,subtitle:fixtureBrowserSubtitle(fixture),intensity:fixtureIntensityPercent(universe,fixture)??0,outputIntensity:fixtureIntensityPercent(outputUniverse,fixture)??0,color:(values.red+values.green+values.blue)>0?rgbToHex(values.red,values.green,values.blue):(fixture.labelColor??'#55e98d'),selected:fixture.selected};})}
          groups={fixtureGroups.map((group)=>({id:group.id,name:group.name,fixtureIds:fixturesInGroup(patch,group).map((fixture)=>fixture.id),intensity:Math.round(groupMasters[group.id]??group.masterDefault),color:group.labelColor,selected:selectedGroupId===group.id}))}
          looks={allLooks.map((look)=>({id:look.id,name:look.name,color:rgbToHex(look.values.red,look.values.green,look.values.blue)}))}
          effects={EFFECT_PRESETS.map((effect)=>({id:effect.id,name:effect.name,active:activeEffect===effect.id,momentary:effect.momentary,color:effect.id==='blinder'?'#ffffff':effect.id==='lightning'?'#c9dcff':effect.id==='rainbow'||effect.id==='color-chase'?'#bc36ff':'#e0a24f'}))}
          onGo={goNextCue}
          ...
          onFixtureLevel={(fixtureId,value)=>{const fixture=patch.find((item)=>item.id===fixtureId);if(fixture)void setFixtureAttribute(fixture,'dimmer',percentToDmx(value));}}
          ...
          onEffectPress={(effectId,momentary)=>{const effect=EFFECT_PRESETS.find((item)=>item.id===effectId);if(effect){if(momentary)startMomentaryEffect(effect.id);else toggleEffect(effect.id);}}}
        ...
        {liveView === 'overrides' && <div className="live-detail-view">
          ...
          <div className="override-layout"><div className="override-fader-bank">{patch.map((fixture)=>{const v=fixtureValues(outputUniverse,fixture);const outputColor=(v.red+v.green+v.blue)>0?rgbToHex(v.red,v.green,v.blue):(fixture.labelColor ?? '#55e98d');return <VerticalFader key={fixture.id} id={`override-${fixture.id}`} name={fixture.name} subtitle={fixtureBrowserSubtitle(fixture)} color={outputColor} value={fixtureIntensityPercent(universe,fixture)} outputValue={fixtureIntensityPercent(outputUniverse,fixture)} selected={fixture.selected} onChange={(value)=>void setFixtureAttribute(fixture,'dimmer',percentToDmx(value))} onSelect={()=>selectFixtureFromConsole(fixture.id,true)} onFx={()=>{setWorkspace('create');setProgramMode('fx');}}/>})}</div><aside className="override-tools"><ColorDeck title="OVERRIDE COLOR" subtitle={selectedFixtureTargets.length?`${selectedFixtureTargets.length} selected`:'Select fixtures'} color={globalColor} disabled={selectedCompatibleColors.length===0} presets={consoleColorPresets} onChange={applyGlobalColor}/><section><header>QUICK COLORS</header><div className="override-quick-grid">{consoleColorPresets.map((preset) => <button key={preset.name} disabled={!selectedCompatibleColors.length} onClick={() => applyGlobalColor(preset.color)}><i style={{background:preset.color}} />{preset.name}</button>)}</div></section><section><header>FX · {liveEffectLabel || 'READY'}</header><div className="override-quick-grid">{EFFECT_PRESETS.slice(0, 12).map((effect) => <button key={effect.id} className={activeEffect===effect.id?'active':''} disabled={!effectSupportedByFixtures(effect.id,selectedFixtureTargets)} onClick={() => toggleEffect(effect.id, selectedFixtureTargets.map((fixture)=>fixture.id))}>{effect.name}</button>)}</div><button onClick={() => stopEffect()}>STOP FX</button></section><button className="console-primary" onClick={()=>{setWorkspace('create');setProgramMode('stage');}}>OPEN FULL PROGRAMMER</button></aside></div>
        </div>}
        ...
        {liveView === 'shortcuts' && <div className="live-detail-view">
          <header><div><span>SHORTCUTS + CONTROL MAPPING</span><h2>Keyboard and MIDI</h2></div><button onClick={()=>setMidiManagerOpen(true)}>OPEN MAPPER · {midiMappings.length}</button></header>
          <div className="shortcut-grid"><section><h3>KEYBOARD</h3><div className="shortcut-row"><kbd>SPACE</kbd><span>GO / next cue</span></div><div className="shortcut-row"><kbd>→</kbd><span>GO / next cue</span></div><div className="shortcut-row"><kbd>←</kbd><span>BACK / previous cue</span></div><div className="shortcut-row"><kbd>ESC</kbd><span>Close active calibration panel</span></div></section><section><h3>MIDI ASSIGNER</h3><MidiMappingPanel compact mappings={midiMappings} controls={midiControls} groups={midiControlGroups} newTarget={newMidiTarget} learningId={midiLearnMappingId} onNewTarget={setNewMidiTarget} onAdd={()=>beginMidiAssignment()} onLearn={setMidiLearnMappingId} onRemove={removeMidiAssignment} onTarget={changeMidiAssignmentTarget} onTrigger={changeMidiTriggerValue} onRestoreDefaults={restoreCommonMidiDefaults} onOpenManager={()=>setMidiManagerOpen(true)}/></section></div>
        </div>}

        {liveView === 'settings' && <div className="live-detail-view">
          <header><div><span>LIVE SYSTEM STATUS</span><h2>Connections + safety</h2></div><button onClick={()=>{setWorkspace('settings'); setSettingsView('connections');}}>OPEN CONNECTIONS</button></header>
      ...
      {midiManagerOpen && <div className="midi-manager-overlay" role="dialog" aria-modal="true" aria-label="MIDI assignment manager" onMouseDown={(event)=>{if(event.target===event.currentTarget)setMidiManagerOpen(false);}}>
        <section className="midi-manager-panel">
          <header><div><span>CONTROL ASSIGNMENT</span><h2>MIDI Mapper</h2><small>All assignments remain visible and scroll independently.</small></div><button aria-label="Close MIDI mapper" onClick={()=>setMidiManagerOpen(false)}>×</button></header>
          <MidiMappingPanel mappings={midiMappings} controls={midiControls} groups={midiControlGroups} newTarget={newMidiTarget} learningId={midiLearnMappingId} onNewTarget={setNewMidiTarget} onAdd={()=>beginMidiAssignment()} onLearn={setMidiLearnMappingId} onRemove={removeMidiAssignment} onTarget={changeMidiAssignmentTarget} onTrigger={changeMidiTriggerValue} onRestoreDefaults={restoreCommonMidiDefaults}/>
        </section>
      </div>}

      <footer className="console-footer console-status-strip">
        <div className="status-connections">
          <button className={dmxStatus.connected ? 'healthy' : ''} onClick={() => { setWorkspace('settings'); setSettingsView('connections'); }}><i />DMX <b>{dmxStatus.connected ? 'ONLINE' : 'VIRTUAL'}</b></button>
          <button className={directStatus.clients > 0 ? 'healthy' : ''} onClick={() => { setWorkspace('settings'); setSettingsView('connections'); }}><i />LUMAVIZ <b>{directStatus.clients > 0 ? 'LINKED' : 'READY'}</b></button>
          ...
          <button className={midiStatus.connected ? 'healthy' : ''} onClick={() => { setWorkspace('settings'); setSettingsView('connections'); }}><i />MIDI <b>{midiStatus.connected ? 'ONLINE' : 'OFF'}</b></button>
        </div>
