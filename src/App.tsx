      ...
      {workspace === 'setup' && <section className="console-workspace setup-console">
        ...
        <div className="setup-center console-center">
          ...
          {setupView === 'settings' && <div className="setup-scroll-area settings-console">
            ...
            <section className="console-panel connection-console visualizer-output-console"><header><div><span>VISUALIZER OUTPUT · ART-NET</span><h2>LumaViz</h2></div><b className={settings.visualizerArtNetEnabled ? 'healthy' : ''}>{settings.visualizerArtNetEnabled ? 'Sending' : 'Off'}</b></header><p>LumaRig sends the final resolved output frame after cues, FX, manual overrides, group masters, and grand master. LumaViz listens without owning playback.</p><label className="inspector-toggle"><span>Send Art-Net to LumaViz</span><input type="checkbox" checked={settings.visualizerArtNetEnabled} onChange={(event) => setSettings((current) => ({ ...current, visualizerArtNetEnabled: event.target.checked }))} /></label><label><span>Target IPv4 address</span><input value={settings.visualizerArtNetTarget} placeholder="127.0.0.1" onChange={(event) => setSettings((current) => ({ ...current, visualizerArtNetTarget: event.target.value }))} /></label><small>Same computer: 127.0.0.1 · Another computer: enter the LumaViz machine's LAN IPv4 address.</small><div className="settings-actions"><button onClick={() => setSettings((current) => ({ ...current, visualizerArtNetTarget: '127.0.0.1', visualizerArtNetEnabled: true }))}>Use This Computer</button><button onClick={() => setSettings((current) => ({ ...current, visualizerArtNetEnabled: false }))}>Stop Art-Net</button></div></section>
