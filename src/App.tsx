    ...
    <main className={`console-app workspace-${workspace} ${dmxStatus.blackout ? 'blackout-is-active' : ''}`}>
      ...
      {workspace === 'live' && <section className="live-reference-shell">
        ...
        <div className="live-reference-main">
          ...
          {liveProgrammerOpen&&<section className="live-programmer-drawer"><header><div><span>PROGRAMMER</span><strong>{selectedFixtureTargets.length? `${selectedFixtureTargets.length} selected`:'No selection'}</strong></div><nav>{(['intensity','position','color','beam','fx'] as ControlSurfaceTab[]).map((tab)=><button key={tab} className={controlSurfaceTab===tab?'active':''} onClick={()=>setControlSurfaceTab(tab)}>{tab.toUpperCase()}</button>)}</nav><button onClick={()=>setLiveProgrammerOpen(false)}>×</button></header><div>{controlSurfaceTab==='intensity'&&surfaceParameterControl('dimmer','DIMMER')}{controlSurfaceTab==='position'&&<>{surfaceParameterControl('pan','PAN')}{surfaceParameterControl('tilt','TILT')}</>}{controlSurfaceTab==='color'&&<ColorDeck fixtures={selectedCompatibleColors} universe={universe} presets={consoleColorPresets} onApply={(updates)=>void commitUpdates(updates)} />}{controlSurfaceTab==='beam'&&<>{surfaceParameterControl('zoom','ZOOM')}{surfaceParameterControl('focus','FOCUS')}{surfaceParameterControl('iris','IRIS')}</>}{controlSurfaceTab==='fx'&&<div className="live-programmer-fx">{EFFECT_PRESETS.filter((effect)=>effectSupportedByFixtures(effect.id,selectedFixtureTargets)).map((effect)=>renderEffectButton(effect,true))}</div>}</div></section>}
