...
type ProgramMode = 'programmer' | 'stage';
...
export default function App() {
  ...
  const [setupView, setSetupView] = useState<SetupView>(() => initialConsoleValue('setup', ['fixtures', 'groups', 'stage', 'settings'], 'fixtures'));
  const [programMode, setProgramMode] = useState<ProgramMode>(() => initialConsoleValue('program', ['programmer', 'stage'], 'programmer'));
  ...
  const [newFixtureUniverse, setNewFixtureUniverse] = useState(1);
  ...
  const programTargetIsGroup = Boolean(selectedGroup && selectedGroupFixtures.length && selectedFixtureTargets.length === selectedGroupFixtures.length && selectedFixtureTargets.every((fixture) => selectedGroupFixtures.some((member) => member.id === fixture.id)));
  const programEffectFixtures = programTargetIsGroup ? selectedGroupFixtures : selectedFixtureTargets;
  const programEffectName = programTargetIsGroup
    ...
    <main className={`console-app workspace-${workspace} ${dmxStatus.blackout ? 'blackout-is-active' : ''}`}>
      ...
      {workspace === 'setup' && <section className="console-workspace setup-console">
        <nav className="workspace-subtabs setup-subtabs">{([
          ['fixtures', '1 · Fixtures'], ['groups', '2 · Groups'], ['stage', '3 · Stage'], ['settings', '4 · Connections']
        ...
        <div className="setup-center console-center">
          {setupView === 'stage' && <>
            <section className="build-step-intro stage-build-intro"><span>STEP 3 OF 4 · STAGE</span><h2>Place the rig in the room.</h2><p>Select a fixture or stage object, then move, rotate, aim, or calibrate it. The stage model drives targeting and the monitor view.</p></section><div className="stage-console-toolbar"><div role="toolbar" aria-label="Stage Designer mode">{STAGE_DESIGNER_MODES.map((mode) => <button key={mode.id} className={stageMode === mode.id ? 'active' : ''} onClick={() => setStageMode(mode.id)}>{mode.label}</button>)}</div><span>{stageSettings.unit === 'feet' ? 'FEET' : 'METERS'} · {stageSettings.dimensions.width.toFixed(1)} × {stageSettings.dimensions.depth.toFixed(1)} m</span></div>
          ...
          {setupView === 'fixtures' && <div className="setup-scroll-area">
            <section className="build-step-intro"><span>STEP 1 OF 4 · FIXTURES</span><h2>Tell LumaRig what is in the rig.</h2><p>Add fixtures, set their DMX addresses, and verify the patch here. You should only need to do this when the physical rig changes.</p></section><section className="fixture-selection-toolbar"><div><strong>YOUR FIXTURES</strong><span>{patch.filter((fixture) => fixture.selected).length} of {patch.length} selected · Universe 1 collision checking active</span></div><div><button onClick={selectAllFixtures}>Select All</button><button onClick={clearFixtureSelection}>Clear</button><button className="danger-button" disabled={!patch.some((fixture) => fixture.selected) || patch.filter((fixture) => fixture.selected).length >= patch.length} onClick={deleteSelectedFixtures}>Delete Selected</button></div></section>
            <section className="console-panel batch-fixture-panel"><header><div><span>ADD FIXTURES</span><h2>Patch a batch</h2></div><div className="universe-actions"><b>{settings.universeCount} universe{settings.universeCount === 1 ? '' : 's'}</b><button onClick={() => setSettings((current) => ({ ...current, universeCount: Math.min(16, current.universeCount + 1) }))}>＋ Add Universe</button></div></header><div className="batch-fixture-grid"><label><span>Fixture Profile</span><select value={newProfileId} onChange={(event) => { const profile = findProfile(event.target.value) ?? FIXTURE_LIBRARY[0]; setNewProfileId(profile.id); setNewModeId(profile.modes[0].id); setProfileAcknowledged(false); }}>{FIXTURE_LIBRARY.map((profile) => <option key={profile.id} value={profile.id}>{profile.verified ? '✓' : '△'} {profile.manufacturer} {profile.model}</option>)}</select></label><label><span>Mode</span><select value={newModeId} onChange={(event) => setNewModeId(event.target.value)}>{newProfile.modes.map((mode) => <option key={mode.id} value={mode.id}>{mode.name} · {mode.channelCount}ch</option>)}</select></label><label><span>Base Name</span><input value={newFixtureName} placeholder={newProfile.model} onChange={(event) => setNewFixtureName(event.target.value)} /></label><label><span>Quantity</span><input type="number" min="1" max="64" value={newFixtureQuantity} onChange={(event) => setNewFixtureQuantity(Number(event.target.value))} /></label><label><span>Universe</span><select value={newFixtureUniverse} onChange={(event) => setNewFixtureUniverse(Number(event.target.value))}>{Array.from({ length: settings.universeCount }, (_, index) => <option key={index + 1} value={index + 1}>Universe {index + 1}</option>)}</select></label><label><span>Starting Address</span><input type="number" min="1" max="512" value={newFixtureAddress} onChange={(event) => setNewFixtureAddress(Number(event.target.value))} /></label><label><span>Group</span><select value={newFixtureGroup} onChange={(event) => setNewFixtureGroup(event.target.value)}><option value="">Unassigned</option>{fixtureGroups.map((group) => <option key={group.id} value={group.name}>{group.name}</option>)}</select></label><button className="console-primary add-batch" onClick={addFixture}>Add {Math.max(1, newFixtureQuantity)} Fixture{newFixtureQuantity === 1 ? '' : 's'}</button></div>{!newProfile.verified && <label className="profile-confirm"><input type="checkbox" checked={profileAcknowledged} onChange={(event) => setProfileAcknowledged(event.target.checked)} /> I checked the fixture manual and exact mode.</label>}</section>
        ...
        <aside className="console-inspector setup-inspector">
          {setupView === 'groups' && selectedGroup ? <>
            ...
            <header><span>FIXTURE INSPECTOR</span><strong>{stageFixture.name}</strong><small>{findProfile(stageFixture.profileId)?.manufacturer} {findProfile(stageFixture.profileId)?.model}</small></header>
            <label><span>Name</span><input value={stageFixture.name} onChange={(event) => savePatchedFixture({ ...stageFixture, name: event.target.value })} /></label>
            <div className="inspector-pair"><label><span>Universe</span><input type="number" min="1" value={stageFixture.universe ?? 1} onChange={(event) => savePatchedFixture({ ...stageFixture, universe: Number(event.target.value) })} /></label><label><span>Address</span><input type="number" min="1" max="512" value={stageFixture.address} onChange={(event) => savePatchedFixture({ ...stageFixture, address: Number(event.target.value) })} /></label></div>
            <label><span>Group</span><select value={stageFixture.group} onChange={(event) => savePatchedFixture({ ...stageFixture, group: event.target.value })}><option value="">Unassigned</option>{fixtureGroups.map((group) => <option key={group.id} value={group.name}>{group.name}</option>)}</select></label>
            <div className="inspector-pair"><label><span>Mounting</span><select value={stageFixture.mounting ?? 'hanging'} onChange={(event) => savePatchedFixture({ ...stageFixture, mounting: event.target.value as PatchedFixture['mounting'] })}><option value="hanging">Hanging</option><option value="floor">Floor</option><option value="wall">Wall</option><option value="custom">Custom</option></select></label><label><span>Orientation</span><select value={stageFixture.orientation ?? 'normal'} onChange={(event) => savePatchedFixture({ ...stageFixture, orientation: event.target.value as PatchedFixture['orientation'] })}><option value="normal">Normal</option><option value="inverted">Inverted</option><option value="rotated90">Rotated 90°</option><option value="rotated180">Rotated 180°</option><option value="custom">Custom</option></select></label></div>
      ...
      {workspace === 'program' && <section className="console-workspace program-console">
        ...
        <div className="program-center console-center">
          <section className="create-context-bar"><div><span>CREATE · TARGET</span><strong>{programEffectName}</strong><small>{programEffectFixtures.length ? `${programEffectFixtures.length} fixture${programEffectFixtures.length === 1 ? '' : 's'} ready` : 'Select a fixture or group on the left'}</small></div><div className="create-view-switch"><button className={programMode === 'programmer' ? 'active' : ''} onClick={() => setProgramMode('programmer')}>PROGRAMMER</button><button className={programMode === 'stage' ? 'active' : ''} onClick={() => setProgramMode('stage')}>STAGE</button></div></section>
          {programMode === 'programmer' && <><ColorDeck title="COLOR" subtitle={programEffectName} color={globalColor} disabled={programTargetIsGroup ? !selectedGroup || compatibleColorFixtures(selectedGroupFixtures).length === 0 : selectedCompatibleColors.length === 0} presets={consoleColorPresets} onChange={(color) => programTargetIsGroup && selectedGroup ? applyGroupColor(selectedGroup, color) : applyGlobalColor(color)} /><section className="fader-bank create-intensity-bank"><header><span>INTENSITY</span><strong>{programTargetIsGroup ? 'Group master' : 'Selected fixture brightness'}</strong></header><div>{programTargetIsGroup && selectedGroup ? <VerticalFader id={selectedGroup.id} name={selectedGroup.name} subtitle={`${selectedGroupFixtures.length} fixtures`} color={selectedGroup.labelColor} value={Math.round(groupMasters[selectedGroup.id] ?? selectedGroup.masterDefault)} selected onChange={(value) => applyGroupMaster(selectedGroup, value)} onSelect={() => selectFixtureGroup(selectedGroup.id)} onFx={() => undefined} /> : selectedFixtureTargets.map((fixture) => <VerticalFader key={fixture.id} id={fixture.id} name={fixture.name} subtitle={fixtureBrowserSubtitle(fixture)} color={fixture.labelColor ?? '#55e98d'} value={fixtureIntensityPercent(universe, fixture)} selected onChange={(value) => void setFixtureAttribute(fixture, 'dimmer', percentToDmx(value))} onSelect={() => selectFixtureFromConsole(fixture.id, true)} onFx={() => undefined} />)}</div></section></>}
          {programMode === 'stage' && <><div className="stage-console-toolbar"><div role="toolbar">{STAGE_DESIGNER_MODES.map((mode) => <button key={mode.id} className={stageMode === mode.id ? 'active' : ''} onClick={() => setStageMode(mode.id)}>{mode.label}</button>)}</div><span>{programEffectFixtures.length} targeted</span></div><div className="program-stage">{renderStagePreview(true)}</div></>}
          <LooksStrip looks={allLooks} onApply={runLook} onSave={saveCurrentLook} />
        </div>
        <EffectsPanel title="FX" targetName={programEffectName} fixtures={programEffectFixtures} activeEffect={activeEffect} bpm={effectBpm} depth={effectDepth} disabled={programTargetIsGroup && !selectedGroup?.fxEnabled} onBpmChange={(value) => { setEffectBpm(value); effectBpmRef.current = value; setTempoSource('manual'); }} onDepthChange={(value) => { setEffectDepth(value); effectDepthRef.current = value; }} onStart={(effect) => toggleEffect(effect, programEffectFixtures.map((fixture) => fixture.id))} onPress={(effect) => startMomentaryEffect(effect, programEffectFixtures.map((fixture) => fixture.id))} onRelease={releaseMomentaryEffect} onStop={() => stopEffect()} />
      </section>}
      ...
      {workspace === 'live' && <section className="live-console live-05">
        <header className="live-command-header"><div><span>LIVE · {showFile.name}</span><strong>{dmxStatus.connected ? 'Output online' : 'Virtual output'}</strong></div><div><b>{tempoSource === 'midi' && midiBpm ? midiBpm : effectBpm} BPM</b><b>{globalMaster}% MASTER</b></div></header>
        <div className="live-cue-hero"><span><small>CURRENT</small><strong>{activeCue?.name ?? 'Ready'}</strong><em>{activeCue ? `Cue ${activeCue.number}` : 'No cue running'}</em></span><button className="live-go" onClick={goNextCue} disabled={!nextCue}>GO<small>{nextCue?.name ?? 'End of show'}</small></button><span><small>NEXT</small><strong>{nextCue?.name ?? 'End of show'}</strong><em>{nextCue ? `Cue ${nextCue.number}` : '—'}</em></span></div>
        <div className="live-nav-buttons"><button onClick={goPreviousCue} disabled={!showFile.cues.length}>← PREVIOUS</button><button onClick={goNextCue} disabled={!nextCue}>NEXT →</button></div>
        <div className="live-performance-grid">
          <section className="live-section live-looks"><header><span>LOOKS</span><small>Immediate recall</small></header><div>{allLooks.slice(0, 8).map((look) => <button key={look.id} onClick={() => runLook(look)}><i style={{ background: lookSwatch(look.values) }} /><strong>{look.name}</strong></button>)}</div></section>
        ...
        <details className="live-section live-control-bank live-override-drawer">
          <summary><span>MANUAL OVERRIDES</span><small>Fixture and group faders</small></summary>
        ...
        <section className="live-master live-safety-strip"><header><span>MASTER</span><strong>{globalMaster}%</strong></header><input type="range" min="0" max={settings.masterLimit} value={globalMaster} onChange={(event) => applyGlobalMaster(Number(event.target.value))} /><div>{[25, 50, 75, 100].map((value) => <button key={value} onClick={() => applyGlobalMaster(value)}>{value}%</button>)}<button className={dmxStatus.blackout ? 'active danger-button' : 'danger-button'} onClick={toggleBlackout}>{dmxStatus.blackout ? 'RELEASE BLACKOUT' : 'BLACKOUT'}</button></div></section>
