...
type Workspace = 'setup' | 'program' | 'show' | 'live'; // UI labels: BUILD / CREATE / SHOW / LIVE
type SetupView = 'fixtures' | 'groups' | 'stage' | 'connections';
...
export default function App() {
  ...
  const [setupView, setSetupView] = useState<SetupView>(() => initialConsoleValue('setup', ['fixtures', 'groups', 'stage', 'connections'], 'stage'));
  ...
  return (
    <main className={`console-app workspace-${workspace} ${dmxStatus.blackout ? 'blackout-is-active' : ''}`}>
      <header className="console-header">
        <div className="console-brand lumarig-brand"><span className="brand-mark">▲</span><div><strong>LumaRig</strong><small>Modern Lighting.<br/>Built for Real Life.</small></div></div>
        <nav className="console-workspace-tabs" aria-label="Workspace">{([['setup','BUILD'],['program','CREATE'],['show','SHOW'],['live','LIVE']] as Array<[Workspace,string]>).map(([item,label]) => <button key={item} className={workspace === item ? 'active' : ''} onClick={() => setWorkspace(item)}>{label}</button>)}</nav>
        <div className="console-header-status">
          ...
          <button className={`connection-pill ${dmxStatus.connected ? 'online' : ''}`} onClick={() => { setWorkspace('setup'); setSetupView('connections'); }}><i />{dmxStatus.connected ? 'DMX Connected' : 'Virtual Output'}</button>
      ...
      {workspace === 'setup' && <section className="console-workspace setup-console">
        <nav className="workspace-subtabs setup-subtabs">{([
          ['fixtures', 'Fixtures'], ['groups', 'Groups'], ['stage', 'Stage'], ['connections', 'Connections']
        ...
        <div className="setup-center console-center">
          ...
          {setupView === 'connections' && <div className="setup-scroll-area settings-console">
            ...
            <section className="console-panel connection-console vizbridge-console"><header><div><span>VISUALIZER ROUTING</span><h2>VizBridge</h2></div><b className={settings.visualizerArtNetEnabled && !artNetTelemetry.lastError ? 'healthy' : ''}>{settings.visualizerArtNetEnabled ? 'Armed' : 'Standby'}</b></header><p>Use VizBridge when LumaViz or another visualizer needs a dedicated Art-Net bridge. LumaRig remains the source of truth and sends the final resolved output over UDP 6454.</p><div className="artnet-health-grid"><div><span>INPUT</span><strong>LumaRig resolved DMX</strong></div><div><span>OUTPUT</span><strong>Art-Net · UDP 6454</strong></div><div><span>LOCAL BRIDGE</span><strong>127.0.0.1</strong></div><div><span>FRAMES</span><strong>{artNetTelemetry.framesSent.toLocaleString()}</strong></div></div><div className="settings-actions"><button className="console-primary" onClick={() => setSettings((current) => ({ ...current, visualizerArtNetTarget: '127.0.0.1', visualizerArtNetEnabled: true }))}>Route to VizBridge</button><button onClick={() => setSettings((current) => ({ ...current, visualizerArtNetEnabled: false }))}>Stop Bridge Feed</button></div><small>For VizBridge on another machine, use that machine's LAN IPv4 in the LumaViz Art-Net panel below. No physical DMX output is changed.</small></section>
      ...
      {workspace === 'program' && <section className="console-workspace program-console">
        ...
        <div className="program-center console-center">
          <header className="workspace-page-heading"><div><span>CREATE</span><h2>Build the lighting</h2><small>Program fixtures and groups, shape color and movement, then save the result as a reusable look.</small></div><b>{selectedFixtureTargets.length} TARGETED</b></header><nav className="workspace-subtabs program-subtabs">{([['stage', 'PROGRAMMER'], ['faders', 'FIXTURES'], ['groups', 'GROUPS']] as Array<[ProgramMode, string]>).map(([id, label]) => <button key={id} className={programMode === id ? 'active' : ''} onClick={() => setProgramMode(id)}>{label}</button>)}</nav>
      ...
      {workspace === 'live' && <section className="live-console">
        <header className="workspace-page-heading live-heading"><div><span>LIVE</span><h2>Playback Console</h2><small>Run the show, busk fixtures and groups, trigger performance FX, and keep master control visible.</small></div><b className={dmxStatus.connected ? 'healthy' : ''}>{dmxStatus.connected ? 'OUTPUT ONLINE' : 'VIRTUAL OUTPUT'}</b></header>
