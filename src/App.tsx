    ...
    <main className={`console-app workspace-${workspace} ${dmxStatus.blackout ? 'blackout-is-active' : ''}`}>
      ...
      {(workspace === 'setup' || workspace === 'program') && <section className="persistent-control-surface">
        ...
        <div className={`surface-controls surface-${controlSurfaceTab}`}>
          ...
          {controlSurfaceTab === 'position' && (controlSurfaceMode === 'xy' ? <div className="surface-xy-pad" onPointerDown={(event) => { event.currentTarget.setPointerCapture(event.pointerId); applySurfaceXY(event.clientX,event.clientY,event.currentTarget); }} onPointerMove={(event) => { if (event.currentTarget.hasPointerCapture(event.pointerId)) applySurfaceXY(event.clientX,event.clientY,event.currentTarget); }}><i /><span>PAN</span><b>TILT</b></div> : controlSurfaceMode === 'palettes' ? <div className="surface-palette-bank">{(showFile.positionPalettes ?? []).map((palette) => <button key={palette.id} onClick={() => void recallPositionPalette(palette)}><i>◎</i><strong>{palette.name}</strong><small>{palette.kind}</small></button>)}{!(showFile.positionPalettes ?? []).length && <small>No position palettes saved yet.</small>}</div> : <div className="surface-parameter-bank">{surfaceParameterControl('pan','PAN')}{surfaceParameterControl('tilt','TILT')}{selectedCapabilities.has('panFine') && surfaceParameterControl('panFine','PAN FINE')}{selectedCapabilities.has('tiltFine') && surfaceParameterControl('tiltFine','TILT FINE')}</div>)}
