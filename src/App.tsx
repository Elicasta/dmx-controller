...
export default function App() {
  ...
  async function checkForUpdates(silent = false) {
    ...
    try {
      ...
      if (update) {
        ...
        if (!silent) setMessage(`DMX Controller ${update.version} is ready to install.`);
      } else {
        setUpdateInfo(null);
        setUpdateStatus('current');
        if (!silent) setMessage(`DMX Controller ${appVersion} is up to date.`);
      }
    } catch (error) {
      if (silent) {
  ...
  async function installAvailableUpdate() {
    ...
    if (!window.confirm(`Install DMX Controller ${updateInfo.version} now? The app will restart.`)) return;
    setUpdateStatus('installing');
    setUpdateError('');
    setMessage(`Preparing DMX Controller ${updateInfo.version}. Lighting output will be safely disconnected before restart.`);
    try {
      stopFade();
      stopEffect(false);
      setAudioArmed(false);
      ...
      setMessage('Update installed. Restarting DMX Controller…');
    } catch (error) {
      const detail = String(error);
      setUpdateError(detail);
      setUpdateStatus('error');
