...
export default function App() {
  ...
  return (
    <main className={`console-app workspace-${workspace} ${dmxStatus.blackout ? 'blackout-is-active' : ''}`}>
      ...
      {workspace === 'live' && <section className="live-console live-v3-console">
        <header className="live-v3-header"><div><span>LIVE PERFORMANCE</span><strong>{showFile.name}</strong></div><div className="live-v3-status"><span className={dmxStatus.connected ? 'healthy' : ''}>● {dmxStatus.connected ? 'DMX ONLINE' : 'VIRTUAL'}</span><span>U{selectedUniverse}</span><span>{tempoSource === 'midi' ? 'MIDI' : 'INTERNAL'} · {tempoSource === 'midi' && midiBpm ? midiBpm : effectBpm} BPM</span></div><div className="live-safety"><button className={dmxStatus.blackout ? 'blackout active' : 'blackout'} onClick={toggleBlackout}>{dmxStatus.blackout ? 'RELEASE BLACKOUT' : 'BLACKOUT'}</button></div></header>
        <div className="live-cue-hero live-v3-cue-hero"><span><small>CURRENT CUE</small><strong>{activeCue?.name ?? 'Ready'}</strong><em>{activeCue ? `Cue ${activeCue.number}` : 'No cue running'}</em></span><button className="live-go" onClick={goNextCue} disabled={!nextCue}>GO<small>{nextCue?.name ?? 'End of show'}</small></button><span><small>NEXT CUE</small><strong>{nextCue?.name ?? 'End of show'}</strong><em>{nextCue ? `Cue ${nextCue.number}` : '—'}</em></span></div>
        <div className="live-nav-buttons live-v3-nav"><button onClick={goPreviousCue}>← PREVIOUS</button><button onClick={() => setWorkspace('show')}>SHOW CONTROL</button><button onClick={goNextCue} disabled={!nextCue}>NEXT →</button></div>
        <section className="live-section live-fx"><header><div><span>PERFORMANCE FX</span><small>Fast access · selected fixtures</small></div>{activeEffect && <button onClick={() => stopEffect()}>Stop FX</button>}</header><div>{EFFECT_PRESETS.filter((effect) => ['bump', 'blinder', 'strobe', 'pulse', 'sweep', 'lightning', 'finale'].includes(effect.id) && effectSupportedByFixtures(effect.id, selectedFixtureTargets)).map((effect) => renderEffectButton(effect, true))}</div></section>
        <section className="live-section live-control-bank live-v3-control-bank">
        ...
        <section className="live-master live-v3-master"><header><span>GRAND MASTER</span><strong>{globalMaster}%</strong></header><div className="master-state"><small>{dmxStatus.blackout ? 'OUTPUT BLACKED OUT' : 'LIVE OUTPUT'}</small></div><input type="range" min="0" max={settings.masterLimit} value={globalMaster} onChange={(event) => applyGlobalMaster(Number(event.target.value))} /><div>{[0, 25, 50, 75, 100].map((value) => <button key={value} onClick={() => applyGlobalMaster(value)}>{value}%</button>)}</div></section>
        <footer className="live-health live-v3-health"><span className={dmxStatus.connected ? 'healthy' : ''}>● {dmxStatus.connected ? 'DMX ONLINE' : 'VIRTUAL OUTPUT'}</span><span className={midiStatus.connected ? 'healthy' : ''}>● MIDI {midiStatus.connected ? 'CONNECTED' : 'OFFLINE'}</span><span>{tempoSource === 'midi' ? 'MIDI CLOCK' : 'INTERNAL'} · {tempoSource === 'midi' && midiBpm ? midiBpm : effectBpm} BPM</span><span>{formatShowTime(externalSongPositionMs || showTrackPositionMs)}</span></footer>
      </section>}
